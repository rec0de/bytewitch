...
class BPList17 : ParseCompanion() {
    ...
    fun parse(bytes: ByteArray, sourceOffset: Int): BPListObject {
        ...
        if(magic.decodeToString() != "bplist17")
            throw Exception("not a bplist17")
    ...
    private fun readObjectFromOffset(bytes: ByteArray, offset: Int): BPListObject {
        ...
        val parsed = when(objectByte) {
            ...
            in 0x40 until 0x50 -> {
                ...
                return BPData(data, Pair(offset, effectiveOffset+byteLen))
            }
            // Unicode string
            in 0x60 until 0x70 -> {
                ...
                BPUnicodeString(string, Pair(offset, effectiveOffset+charLen*2))
            }
            // Ascii string
            in 0x70 until 0x80 -> {
                // length bits encode character count, if all ones additional length integer follows
            ...
            in 0x80 until 0x90 -> {
                ...
                println("parsing reference object at index $objAddr")

                BPReferenceObject(readObjectFromOffset(bytes, objAddr), Pair(offset, effectiveOffset+sizeFieldLength))
            }
            // Array
            in 0xa0 until 0xb0 -> {
            ...
            in 0xd0 until 0xe0 -> {
                ...
                var currentOffset = offset + 1 + 8
                val dictStartOffset = currentOffset
    ...
    private fun getFillAwareLengthAndOffset(bytes: ByteArray, offset: Int): Pair<Int, Int> {
        ...
        // TODO should also check if bytes[offset+1] & 0x10 = 0x10
...
data class BPReferenceObject(
    val referencedObject: BPListObject,
    override val sourceByteRange: Pair<Int, Int>
) : BPListObject() {

    override fun renderHtmlValue(): String {
        return """<div class="bpvalue bpreference" $byteRangeDataTags>→ ${referencedObject.renderHtmlValue()}</div>"""
    }

    override fun toString(): String {
        return "ref → $referencedObject"
    }
}